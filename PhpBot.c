...
int PhpBot::getPlayerNum()
{
  return playerNum;
}

void PhpBot::setPlayerNum(int thisNum)
{
  if(thisNum < 0)
    playerNum = -1;
  else
...
Move PhpBot::chooseMove(Board thisBoard){  
...
float PhpBot::Evaluate(Board thisBoard) {
...
vector<Move> PhpBot::ReorderMoves(vector<Move> ListOfMoves, Board thisBoard) {
...
float PhpBot::alphaBeta(Board thisBoard, int depth, float alpha, float beta) {
  int PlayerToMove = thisBoard.getWhoseTurn();
